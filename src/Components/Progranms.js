      ...
      <div className="col-md-3 d-flex flex-column justify-content-center">
        <div className="card mb-4 box-shadow">
          ...
          <div className="card-body">
            <h5 className="card-title bluec">{title}</h5>
            <p className="card-text">{description}</p>
            <div className={`read-more ${showMore ? "show" : "hide"}`}>
              <p>
                {moreText}
              </p>
            </div>
            <Button variant="outline-primary" className='pbutton read-more-button' onClick={toggleShowMore}>
...
const Programs = ({ limit }) => {
  const columns = [
    ...
    },
    {
      ...
      moreText: "Lorem ipsum dolor sit FOUR"
    },
    {
    ...
    <div className="container" fluid>
      ...
      <div className="row">
        {limitedColumns.map((column, index) => (
          <Column key={index} title={column.title} imgSrc={column.imgSrc} description={column.description} moreText={column.moreText} />
        ))}
      </div>
    </div>
  );
